...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    not_literals = [~l for l in literals]
    comb = list(itertools.combinations(not_literals, 2))
    return conjoin([disjoin(list(c)) for c in comb])
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    # following the provided pseudocode:
    
    for x, y in all_coords:
        pacphysics_sentences.append(PropSymbolExpr(wall_str, x, y) >> ~PropSymbolExpr(pacman_str, x, y, time=t))
        
    pacphysics_sentences.append(exactlyOne(list(PropSymbolExpr(pacman_str, x, y, time=t) for x, y in non_outer_wall_coords)))
    
    pacphysics_sentences.append(exactlyOne(list(PropSymbolExpr(action, time=t) for action in DIRECTIONS)))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            pacman_locations.append(PropSymbolExpr(pacman_str, x, y, time=t))
        KB.append(exactlyOne(pacman_locations))
                
        directions = []
        for a in actions:
            directions.append(PropSymbolExpr(a, time=t))
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        food_status = []
        for x, y in food:
            # pass not-expression and then conjoin them
            # to check if all food has been eaten
            food_status.append(~PropSymbolExpr(food_str, x, y, time=t))
        is_goal = findModel(conjoin(KB) & conjoin(food_status))
        if is_goal:
            return extractActionSequence(is_goal, actions)
        
        for x, y in food:
            # if food is present at (x,y,t) and pacman is at (x,y,t) then at (x,y,t+1) food won't be there (pacman ate it)
            # if food isn't present at (x,y,t) then food won't be present at (x,y,t+1)
            KB.append(PropSymbolExpr(food_str, x, y, time=t) & PropSymbolExpr(pacman_str, x, y, time=t) | ~PropSymbolExpr(food_str, x, y, time=t)
...
def localization(problem, agent) -> Generator:
    ...
    # find where there are no walls
    not_walls = [w for w in all_coords if w not in walls_list]
...
def mapping(problem, agent) -> Generator:
    ...
    start_pos = PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0)
    KB.append(start_pos)
    KB.append(start_pos >> ~PropSymbolExpr(wall_str, pac_x_0, pac_y_0)) # wall at that position
...
def slam(problem, agent) -> Generator:
    ...
    start_pos = PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time=0)
    KB.append(start_pos)
    KB.append(start_pos >> ~PropSymbolExpr(wall_str, pac_x_0, pac_y_0))
